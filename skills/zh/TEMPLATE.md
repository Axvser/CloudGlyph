...
⚙ **页面聚焦上限（默认拆分）：** 功能页不是单一的巨型文档——拆分为默认动作，而非例外。当单页预计超过**约 300 行**或涵盖**3 个以上不同主题**时应拆分，按能力/操作/端点边界拆分（如 `00_{功能}/00_{操作}/index.md`）。父级 `index.md` 变成简短概览，并用【链接与导航】中的跨页链接语法链接其子页。拿不准时优先拆分。在 QuickStart / API / SE 分析中统一适用。
...
1. **外部链接** — 目标以 `https://`、`http://` 或 `mailto:` 开头，由系统浏览器打开。谨慎使用。
